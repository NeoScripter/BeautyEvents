jQuery(document).ready(function ($) {
    $("#registration-form").on("submit", function (e) {
        ...
        $.ajax({
            ...
            data: formData + "&action=ajax_user_registration&security=" + ajax_vars.registration_nonce,
    ...
    function initFormValidation() {
        ...
        const errorMessage = $(".phone-error-message");
        const phonePattern = /^(?:\+?(\d{1,3})[-.\s]?)?(?:\(?(\d{3})\)?[-.\s]?)?(\d{3})[-.\s]?(\d{4})$/;

        function validatePhone() {
            const phoneValue = phoneField.val().trim();
            const cleanedPhoneValue = phoneValue.replace(/\D/g, "");
            if (phoneValue === "" || (phonePattern.test(phoneValue) && cleanedPhoneValue.length >= 10)) {
                errorMessage.hide();
